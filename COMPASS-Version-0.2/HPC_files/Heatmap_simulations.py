...
def plot3D(criterion = 'IC', ntrials = np.arange(80, 1000, 160), 
    ...
    if criterion == 'IC': 
        ...
        title = 'P(Ga<Gb|Ga<Gb) with alpha = {} and nreps = {}'.format(typeIerror, nreps)
    else: 
    ...
    elif criterion == 'GD': axes.set_title('ES = {}, Sp = {}'.format(ES, sd))
    
    fig.set_size_inches((8, 6), forward=False)
    fig.tight_layout()
    fig.savefig(os.path.join(plot_folder, 'Heatmap{}{}SD{}{}nreps.png'.format(criterion, sd, ES_text, nreps)))
...
Power_df = plot3D(criterion = criterion, sd = sd, ES = ES, nreps = nreps, tau = 0.75)
