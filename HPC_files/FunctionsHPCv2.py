...
def create_design(ntrials = 480, nreversals = 1, reward_probability = 0.8, irep = 0, data_dir = os.getcwd()):
    ...
    """
# Create the template for the design: a numpy array of shape (ntrials X 5)
    design = np.empty([ntrials, 5])
...
# Column 3: define the correct response at each trial
    ...
    for trial in range(design.shape[0]): 
        if design[trial, 0] == 1: design[trial, 3] = design[trial, 1]
        else: design[trial, 3] = (not design[trial, 1])*1
