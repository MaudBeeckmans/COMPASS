...
from FunctionsHPC import create_design, correlation_repetition, groupdifference_repetition, check_input_parameters
...
def power_estimation_correlation(npp = 30, ntrials = 480, nreversals = 12, cut_off = 0.7, high_performance = False, 
    ...
    while rep < nreps: 
        ...
        for logical_cpus in range(n_cpu):
            
            proc = Process(target = correlation_repetition, args =(inverseTemp_distribution, LR_distribution, npp, ntrials, 
                                                         start_design, rep, nreps, n_cpu, data_dir, q))
            proc.start()
    ...
    power_estimate = np.mean((Statistics >= cut_off)*1)
    ...
    print(Statistics)
    return Statistics, power_estimate
